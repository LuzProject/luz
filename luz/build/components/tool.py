...
class Tool(ModuleBuilder):
    ...
    def __compile_tool_file(self, file) -> bool:
        ...
        log(f'Compiling "{file_formatted}"...', f"🔨 {self.module.abbreviate()}", self.luz.lock)
    ...
    def __stage(self):
        ...
        if self.module.install_dir is None:
            ...
            if self.meta.rootless:
                warn(f'Custom install directory was specified, and rootless is enabled. Prefixing path with /var/jb.', f"⚠️ {self.module.abbreviate()}")
