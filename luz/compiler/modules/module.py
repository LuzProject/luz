...
class Module:
    def __init__(self, module: dict, key: str, luzbuild):
        ...
        self.swift_flags = get_from_cfg(
            luzbuild,
            f"modules.{key}.swiftflags",
            f"modules.types.{self.type}.swiftflags",
        )

        # optimization
        self.optimization = get_from_cfg(
            luzbuild,
            f"modules.{key}.optimization",
            f"modules.types.{self.type}.optimization",
        )
        ...
        self.only_compile_changed = bool(
            get_from_cfg(
                luzbuild,
                f"modules.{key}.onlyCompileChanged",
                f"modules.types.{self.type}.onlyCompileChanged",
            )
        )
        ...
        bridging_headersD = list(
            get_from_cfg(
                luzbuild,
                f"modules.{key}.bridgingHeaders",
        ...
        self.files = self.hash_files(
            ...
            True if self.type == "tweak" else False,
        )
    ...
    def linker(self, compile_type: str = "dylib"):
        ...
        for arch in self.luzbuild.archs:
            try:
                self.luzbuild.c_compiler.compile(
                    resolve_path(f"{self.dir}/obj/{self.name}/{arch}/*.o"),
                    outfile=f"{self.dir}/obj/{self.name}/{arch}/{self.name}",
        ...
        try:
            # run ldid
            check_output(
                f"{self.luzbuild.ldid} {self.entflag}{self.entfile} {out_name}",
                shell=True,
            )
        except:
