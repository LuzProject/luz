...
class Preferences(Module):
    def __init__(self):
        ...
        # add values to dict
        self.dict.update({"modules": {self.name: {"type": "preferences", "files": [f"Sources/{self.prefix}RootListController.m" if self.srctype != "swift" else f"Sources/RootListController.swift"]}}})
        ...
        # folder
        self.folder = resolve_path(self.__ask_for("folder for project", self.control["name"] if self.control is not None else self.name))
        # write to yaml
    ...
    def after_untar(self):
        ...
        if self.srctype == "objc":
            rename(f"{self.folder}/Sources/XXXRootListController.m", f"{self.folder}/Sources/{self.prefix}RootListController.m")
            rename(f"{self.folder}/Sources/XXXRootListController.h", f"{self.folder}/Sources/{self.prefix}RootListController.h")
            # fix files
            ...
            rename(f"{self.folder}/Sources/X-Bridging-Header.h", f"{self.folder}/Sources/{self.name}-Bridging-Header.h")

        # move plist
        rename(f"{self.folder}/layout/Library/PreferenceLoader/Preferences/XXX.plist", f"{self.folder}/layout/Library/PreferenceLoader/Preferences/{self.name}.plist")
        # format plists
        content = open(f"{self.folder}/layout/Library/PreferenceLoader/Preferences/{self.name}.plist", "r").read()
        with open(f"{self.folder}/layout/Library/PreferenceLoader/Preferences/{self.name}.plist", "w") as f:
