...
def main():
    ...
    if args.update:
        ...
        try:
            command_wrapper(f"{executable} -m pip uninstall -y luz pydeb pyclang && {executable} -m pip install --user https://github.com/LuzProject/luz/archive/refs/heads/{args.ref}.zip")
        except Exception as err:
    ...
    log("Installing luz...")
    try:
        command_wrapper(f"{executable} -m pip install --user https://github.com/LuzProject/luz/archive/refs/heads/{args.ref}.zip")
    except Exception as err:
